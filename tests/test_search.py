...
def test_search_interactions(client: TestClient):
    ...
    p1 = client.post(
        "/api/v1/patients/",
        json={"first_name": "Interact", "last_name": "One", "date_of_birth": "2000-01-01", "gender": "Male"}
    ).json()["id"]
    
    p2 = client.post(
        "/api/v1/patients/",
        json={"first_name": "Interact", "last_name": "Two", "date_of_birth": "2000-01-01", "gender": "Female"}
    ).json()["id"]

    # Setup: Create 3 Interactions
    # P1: Healthy
    client.post("/api/v1/interactions/", json={"patient_id": p1, "outcome": "Healthy", "notes": "1"})
    # P1: Critical
    client.post("/api/v1/interactions/", json={"patient_id": p1, "outcome": "Critical", "notes": "2"})
    # P2: Healthy
    client.post("/api/v1/interactions/", json={"patient_id": p2, "outcome": "Healthy", "notes": "3"})
